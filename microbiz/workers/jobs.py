"""
Job board scraping worker using Swarms Agent.

This worker extracts job postings with tech stack analysis.
"""
...
from modules.tools import job_board_tool
from modules.parser import parse_agent_output, parse_jobs
...
def run_worker(global_config: Dict[str, Any] = None) -> int:
    """Run the job board scraping worker."""
    try:
        ...
        job_sources = sources_config.get("jobs", {}).get("sources", [])
        if not job_sources:
            job_sources = [{"url": "https://example-jobs.com/latest", "name": "default"}]

        tools = [job_board_tool]
        ...
        agent = Agent(
            agent_name="Job Board Scraper",
            model_name=model_name,
            system_prompt=(
                "You are a job board scraping agent. "
                "Your task is to extract job postings with their required tech stack, salary ranges, and location. "
                "Return the data as a JSON string with a 'jobs' array containing objects with these fields."
            ),
            tools=tools,
            max_loops=3,
        ...
        for source in job_sources:
            ...
            try:
                ...
                task = (
                    f"Scrape job postings from {source_url}. "
                    "Extract all job postings with their title, company, required tech stack, salary range, and location. "
                    "Return the data as a JSON string with this structure: "
                    '{"jobs": [{"title": "...", "company": "...", "tech_stack": "...", "salary": "...", "location": "..."}]}'
                )

                logger.info(f"Running job board scraping agent for {source_name}...")
                output = agent.run(task)

                parsed_output = parse_agent_output(output)
                structured_data = parse_jobs(parsed_output)

                validation_result = validate_data(structured_data, "jobs")
                valid_data = validation_result.get("valid_items", [])
                quality_data = filter_by_quality(valid_data, "jobs", min_quality=0.7)
        ...
        if not all_data:
            logger.warning("No valid job data collected")
            return 1
        ...
        output_data = {
            ...
            "statistics": {
                ...
                "sources_processed": len([s for s in job_sources if s.get("enabled", True)]),
                "errors": len(errors),
            },
        }
        if summary:
        ...
        output_path = f"output/jobs/{timestamp}.json"
        gcs_bucket = config.get("storage", {}).get("gcs_bucket")

        success = save_output(output_path, output_data, gcs_bucket)
        if success:
            logger.info(f"Job data saved to {output_path}")
            return 0
        else:
            logger.error("Failed to save job data")
            return 1

    except Exception as e:
        logger.error(f"Error in job board worker: {e}")
        return 1
