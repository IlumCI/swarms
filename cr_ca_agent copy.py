...
class CRCAAgent:
    # CR-CA agent (compact; docstrings removed)
    ...
    def detect_confounders(self, treatment: str, outcome: str) -> List[str]:
        confounders = []
        t_anc = set(nx.ancestors(self.causal_graph, treatment))
        o_anc = set(nx.ancestors(self.causal_graph, outcome))
        for anc in t_anc & o_anc:
            if nx.has_path(self.causal_graph, anc, treatment) and nx.has_path(self.causal_graph, anc, outcome):
                confounders.append(anc)
        return confounders
    ...
    def _standardize_state(self, state: Dict[str, float]) -> Dict[str, float]:
        ...
        for k, v in state.items():
            ...
            z[k] = (v - s["mean"]) / s["std"] if s and s.get("std", 0.0) > 0 else v
        return z

    def _destandardize_value(self, var: str, z_value: float) -> float:
        s = self.standardization_stats.get(var)
        return z_value * s["std"] + s["mean"] if s and s.get("std", 0.0) > 0 else z_value
    ...
    def _predict_outcomes(
        ...
        for node in nx.topological_sort(self.causal_graph):
            ...
            for p in parents:
                ...
                strength = self.causal_graph[p][node].get('strength', 0.0)
                s += pz * strength
            z_pred[node] = s
    ...
    def identify_adjustment_set(self, treatment: str, outcome: str) -> List[str]:
        # back-door adjustment heuristic
    ...
    def estimate_ate(
        ...
        # estimate ATE via OLS in standardized space
        if df is None or len(df) == 0:
            return 0.0
    ...
    def estimate_cate(
        ...
        # heterogeneous effects via binning
    ...
    def detect_change_points(self, series: List[float], threshold: float = 2.5) -> List[int]:
        # simple CUSUM-like change detector
    ...
    def learn_structure(
        ...
        # structure learning via correlation threshold
        if df is None or len(df) == 0:
            return
    ...
    def sample_joint_interventions_gaussian_copula(
        ...
        # sample joint interventions from Gaussian copula
    ...
    def plan_interventions(
        ...
        # risk-aware grid search planner (compact)
        if not candidates:
            return []
    ...
    def causal_explain_ace(self, child: str) -> Dict[str, float]:
        # decompose child prediction into parent contributions
        parents = list(self.causal_graph.predecessors(child))
        if not parents:
            return {}
    ...
    def identifiability_report(self, treatment: str, outcome: str) -> str:
        # identifiability report (back-door check)
    ...
    def identify_instruments(self, treatment: str, outcome: str) -> List[str]:
        # heuristic IV identification: parents of T without path to Y
        if treatment not in self.causal_graph or outcome not in self.causal_graph:
            return []
    ...
    def estimate_2sls(
        ...
        # two-stage least squares (compact)
    ...
    def find_optimal_interventions(
        ...
        # search optimal single-variable interventions
    ...
    def _synthesize_causal_analysis(self, task: str) -> str:
        # synthesize final analysis via agent
        synthesis_prompt = f"""
    ...
    def get_causal_graph_visualization(self) -> str:
        # text representation of causal graph
    ...
    def analyze_cascading_chain_reaction(
        ...
        # analyze cascading chain reactions
    ...
    def multi_layer_whatif_analysis(
        ...
        # nested what-if analysis (compact)
        results: List[Dict[str, Any]] = []
    ...
    def deep_root_cause_analysis(
        ...
        # deep backward root-cause tracing (compact)
    ...
    def explore_alternate_realities(
        ...
        # explore alternate realities (Monte Carlo search)
    ...
    def historical_pattern_matching(
        ...
        # historical pattern matching (compact)
    ...
    def infinite_nesting_root_cause(
        ...
        # recursive root-cause tracing (compact)
    ...
    def optimal_intervention_sequence(
        ...
        # greedy sequence search for interventions (compact)
    ...
    def meta_learn_intervention_strategy(
        ...
        # meta-learn intervention patterns from history
    ...
    def recursive_alternate_reality_search(
        ...
        # recursive alternate reality search (compact)
        if path is None:
            path = []
    ...
    def adversarial_nested_analysis(
        ...
        # adversarial nested analysis (compact)
    ...
    def multi_objective_infinite_nesting(
        ...
        # multi-objective beam search (compact)
    ...
    def shapley_value_attribution(
        ...
        # Shapley value attribution (compact)
